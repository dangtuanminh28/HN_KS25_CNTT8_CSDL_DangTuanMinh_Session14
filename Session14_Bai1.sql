...
-- Phần A
CALL PayHospitalFee(1, 20000000);
-- Kiểm tra số dư ví (Tiền đã bị trừ mất 200k)
SELECT patient_id, balance, status FROM Wallets WHERE patient_id = 1;
-- Kiểm tra công nợ (Nợ vẫn giữ nguyên, không hề giảm)
SELECT patient_id, total_due FROM Patient_Invoices WHERE patient_id = 1;

/*
Giải thích:
Code trên do vi phạm tính chất Atomicity (tính nguyên tử), tính chất này phải thực hiện theo tất cả hoặc ko có gì
